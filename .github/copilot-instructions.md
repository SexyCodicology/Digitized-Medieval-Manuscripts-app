...
- **Link status:** `is_disabled` and `last_checked` are optional. The weekly check stamps `last_checked` when it confirms a working collection URL, and uses it to date a confirmed broken link. If `is_disabled` is `true`, confirm `last_checked` is present and is a real ISO 8601 date (`YYYY-MM-DD`). Reject a broken-link claim backed only by a timeout, 403, 429, or 5xx response — those describe the server, not the collection. A collection that moved needs a corrected `website`, not a broken-link mark.
